...
static ESL_OPTIONS options[] = {
  ...
  { "--partial",    eslARG_STRING,     NULL, NULL,            NULL,      NULL,         NULL,      NULL, "only calibrate for certain searches.  lc,gc,li,gi,hmm.  l=local, g=global, c=CYK, i=inside, hmm=HMM (not tested with --mpi)",3},
  ...
  { "--print-env",  eslARG_STRING,     NULL, NULL,     NULL,             NULL,         NULL,      NULL, "print values of environment vars, for help with cluster scripts", 5 },
  { "--stop-file",  eslARG_STRING,     NULL, NULL,     NULL,             NULL,         NULL,      NULL, "print values of environment vars, for help with cluster scripts", 5 },
#ifdef HMMER_THREADS 
...
static int  should_we_calibrate_this_expmode(const ESL_GETOPTS *go,int expmode);
...
main(int argc, char **argv)
{
  ...
  if (esl_opt_IsUsed(go, "--stop-file")) {
    char *stopFileName=esl_opt_GetString(go,"--stop-file");
    FILE *f=fopen(stopFileName,"w+t");
    if (f==NULL) {
  ...
  while (qhstatus == eslOK) {
	...
	for(exp_mode = 0; exp_mode < EXP_NMODES; exp_mode++) {
	  ...
	  if(exp_mode > 0 && (! ExpModeIsLocal(exp_mode-1)) && ExpModeIsLocal(exp_mode)) {
		...
		if (should_we_calibrate_this_expmode(go, exp_mode)) {
...
output_header(FILE *ofp, const ESL_GETOPTS *go, char *cmfile, int available_ncpus, int ncpus)
{
  ...
  if (esl_opt_IsUsed(go, "--print-env")) {
    char *s=esl_opt_GetString(go, "--print-env");
    while (1) {
      char *r=strtok(s,",");
      s=NULL;
      if (r==NULL) {
	break;
      ...
      char *e=getenv(r);
      if (e==NULL) {
	e=(char *)""; /* if not found, return empty string */
      }
      fprintf(ofp,"--print-env,%s,%s,%s\n",r,e,r); /* easy-to-parse output */
    }
    /* always output HOSTNAME, but using result from gethostname */
    ...
    fprintf(ofp,"--print-env,HOSTNAME,%s,HOSTNAME\n",hostname);
  }
...
/* Function: should_we_calibrate_this_expmode
 * 
 * Purpose: interpret the --partial flag (or its absence)
 *          and an expmode (0..EXP_NMODES-1) and return
 *          true if we should calibrate this mode.
 *          calls esl_fatal if there is an error
 * 
 * Returns: 1 to calibrate, 0 to skip
 */
static int
should_we_calibrate_this_expmode(const ESL_GETOPTS *go,int expmode)
{
	if (esl_opt_IsOn(go,"--partial")) {
		const char *mode=esl_opt_GetString(go,"--partial");
		if (strcmp(mode,"lc")==0) {
			return expmode==EXP_CM_LC;
		}
		if (strcmp(mode,"gc")==0) {
			return expmode==EXP_CM_GC;
		}
		if (strcmp(mode,"li")==0) {
			return expmode==EXP_CM_LC || EXP_CM_LI;
		}
		if (strcmp(mode,"gi")==0) {
			return expmode==EXP_CM_GC || EXP_CM_GI;
		}
		if (strcmp(mode,"h")==0 || strcmp(mode,"hmm")==0) {
			return 0; /* don't calibrate CM at all for HMM only */
		}
		esl_fatal("flag parameter \"--partial %s\" is not a recognized option",mode);
		return 0; /* keep compiler happy */
	}
	else {
		/* --partial flag not used, so we should calibrate all modes */
		return 1;
	}
}
