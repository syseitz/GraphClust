...
#ifdef WITHIN_CMFINDER
  if(esl_opt_GetBoolean(go, "--noF5"))    {pli->do_edef = FALSE; pli->do_edefbias=FALSE; } /* zlh */
#endif
...
cm_pli_NewModel(CM_PIPELINE *pli, int modmode, CM_t *cm, int cm_clen, int cm_W, int cm_nbp, P7_OPROFILE *om, P7_BG *bg, float *p7_evparam, int p7_max_length, int64_t cur_cm_idx, ESL_KEYHASH *glocal_kh)
{
  ...
  /* validate that exp mode data is available */
  if(pli->by_E) {
	  if (cm->expA[pli->fcyk_cm_exp_mode]->nrandhits==-1 || cm->expA[pli->final_cm_exp_mode]->nrandhits==-1) {
		  cm_Fail("CM was not sufficiently calibrated for the desired search, because cmcalibrate was used with the --partial flag.  You'll need to re-calibrate the CM for this search, or use different cmsearch/cmscan flags.");
	  }
  }
  ...
  if(pli->mode == CM_SEARCH_SEQS || modmode == CM_NEWMODEL_CM) { 
	...
	pli->T = cm_p7_E2Score(pli->E, pli->Z, p7_max_length, p7_evparam[CM_p7_LFTAU], p7_evparam[CM_p7_LFLAMBDA]);
      ...
      if(pli->by_E) { 
  		if (cm->expA[pli->final_cm_exp_mode]->dbsize==-1) {
			esl_fatal("exp mode %d was not calibrated, but E-values are required",pli->final_cm_exp_mode);
		}
  ...
  for (i = 0; i < np7env; i++) {
    ...
    if(status == eslERANGE) {
      ...
      continue; /* skip envelopes that would require too big of a HMM banded matrix */
		...
		if (cm->expA[pli->fcyk_cm_exp_mode]->dbsize==-1) {
			esl_fatal("exp mode %d was not calibrated, but E-values are required",pli->fcyk_cm_exp_mode);
		}
