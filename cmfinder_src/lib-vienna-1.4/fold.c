...
PUBLIC void  comp_energy(char *string);
PUBLIC void  get_structure(int start, int stop, char* string, char* structure, char backtrack_type);

PUBLIC  int   *bp_energy;
PUBLIC  int   *ml_energy;
...
PRIVATE void  parenthesis_structure(char *structure, int start, int end);
...
void get_structure(int start, int end, char* string, char* structure, char backtrack_type)
{
  int   s, b, mm;  
  int   i, j,k, l, p, q,length, energy, new_c, new;
  int   no_close, type, type_2, tt;
  ...
  length = end;
  memset(base_pair, 0, sizeof(struct bond) * ( 1 + length/ 2));  
  ...
  parenthesis_structure(structure, start, end);
...
PRIVATE void parenthesis_structure(char *structure, int start, int end)
{
  ...
  for (n = 0; n <= end - start; structure[n++] = '.') ;
  structure[end-start + 1] = '\0';

  for (k = 1; k <= base_pair[0].i; k++) {
    structure[base_pair[k].i - start] = '(';
    structure[base_pair[k].j - start] = ')';
  }
}
/*---------------------------------------------------------------------------*/
