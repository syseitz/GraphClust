...
#include  "dist_vars.h"
#include  "utils.h"
static char rcsid[] = "$Id: treedist.c,v 1.1.1.1 2006/05/25 16:33:54 yzizhen Exp $";
...
PUBLIC  void      sprint_aligned_trees(void);

PRIVATE void            tree_dist(int i, int j);
PRIVATE double          edit_cost(int i, int j);
...
PRIVATE  double  **tdist;       /* contains distances between subtrees */
PRIVATE  double  **fdist;       /* contains distances between forests */
...
PUBLIC float tree_edit_distance(Tree *T1, Tree *T2)
{
   ...
   tdist = (double **) space(sizeof(double *) * (n1+1));
   fdist = (double **) space(sizeof(double *) * (n1+1));
   for (i=0; i<=n1; i++) { 
      tdist[i] = (double *) space(sizeof(double) * (n2+1));
      fdist[i] = (double *) space(sizeof(double) * (n2+1));
   }
   ...
   for (i1 = 1; i1 <= T1->keyroots[0]; i1++) {
	 ...
	 tree_dist(i,j);
	 //printf("Aligned tree1 %d \t tree2 %d dist %f\t %f\n", i, j, fdist[i][j], tdist[i][j]);	 	 
      }
   }
...
PRIVATE void tree_dist(int i, int j)
{
  int li,lj,i1,j1,i1_1,j1_1,li1_1,lj1_1;
  double f1,f2,f3,f;
...
PRIVATE void backtracking(void)
{
 ...
 start:
  ...
  while ((i1>=li)&&(j1>=lj)) {
    ...
    f = fdist[i1][j1];  
    if ( abs(f - (fdist[i1_1][j1_1] + edit_cost(i1, j1))) < precision&& lleaf_i1 == li && lleaf_j1 == lj){
      //printf("%d %d %f match at score %f\n", i1, j1, f, edit_cost(i1,j1));
      alignment[0][i1] = j1;
      alignment[1][j1] = i1;
      i1=i1_1; j1=j1_1;
    }
    else if( abs(f - ( fdist[i1_1][j1] + edit_cost(i1, 0))) < precision) {
      //printf("%d %d %f insert tree1 %d\n", i1, j1, f, i1);
      alignment[0][i1]=0;
      i1=i1_1;
    }      
    else if (abs(f - (fdist[i1][j1_1] + edit_cost(0, j1))) <  precision) {
      //printf("%d %d %f insert tree2 %d\n", i1, j1, f, j1);
      alignment[1][j1]=0;
      j1=j1_1;
    }
    else{
